...
def main():
    ...
    for task in tasks:        
        for probe in ['recommendation', 'search']:
            ...
            # for model in ['microsoft/deberta-base']:                
            # for model in ['bert-base-cased', 'microsoft/deberta-base']:                
    ...
    if write_R_with_X_cand_file:
        ...
        random_line = ["search", "random", "microsoft/deberta-base"]
    ...
    #                     (df_lists["task"]==task) & 
    #                     (df_lists["model"]=="microsoft/deberta-base")][metric].values[0]
